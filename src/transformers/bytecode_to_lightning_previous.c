...
VECTOR_INDEX_FUNCTION(Register*, register)
...
void instruction_to_lightning(Executor* E, 
    ...
    #define TEMPORARY_REGISTER() vector_pop(temporary_registers)
    #define PRESERVED_REGISTER() vector_pop(preserved_registers)
    #define CALL(function) \
                if(!vector_empty(temporary_registers)) { \
                    ERROR("Temporary registers in use at call.") \
                } else { \
                    jit_finishi(function); \
                }
    #define RETURN_TEMPORARY_REGISTER(register) vector_push(temporary_registers, register);
    #define RETURN_PRESERVED_REGISTER vector_push(preserved_registers, register);
    ...
    switch (instruction->type){
        ...
        CASE(b_swap)
            ...
            Register temporary=TEMPORARY_REGISTER();
        ...
        CASE(b_load_float)
            ...
            jit_movi_f(JIT_F0, instruction->float_argument);
            jit_str_f(JIT_V2, JIT_F0);
            break;
        CASE(b_load_string)
        ...
        CASE(b_enter_scope)
            ...
            jit_pushargr(JIT_V2);
            jit_pushargr(JIT_V3);
            CALL(inherit_scope_lightning_wrapper);
        ...
        CASE(b_jump_not)
            ...
            jit_prepare();
            jit_pushargr(JIT_V1);
            CALL(is_falsy_lightning_wrapper);
            jit_retval(JIT_V2);
            jit_labels[instruction->uint_argument]=jit_forward();
        ...
        CASE(b_tail_call)
            ...
            jit_pushargr(JIT_R2);
            CALL(call_lightning_wrapper);
            DEREFERENCE(JIT_V2, JIT_V1)
            jit_prepare();
            jit_pushargr(JIT_V1);
            jit_pushargr(OBJECT_STACK_REGISTER);
            jit_pushargi(instruction->uint_argument);
            CALL(stack_remove_items);
            PUSH(JIT_R2)
